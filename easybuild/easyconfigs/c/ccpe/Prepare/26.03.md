...
-   The above steps result in the following definition file (`cpe-26.03.01-SP7-LUMI-26.01.def`):
    ...
    /usr/lib64/libhugetlbfs
    /usr/share/doc/cray-libhugetlbfs
    ...
    # Phase 3 - AOCC
    # 
    # We downloaded the RPM manually. cURL or wget do not work as the EULA needs to be accepted.
    aocc-compiler-5.1.0.sles15-1.x86_64.rpm work/aocc-compiler-5.1.0.sles15-1.x86_64.rpm
    /opt/cray/pe/lmod/modulefiles/core/aocc/5.0.0.lua /opt/cray/pe/lmod/modulefiles/core/aocc/5.1.0.lua
    ...
    # Install aocc
    # It complains about libxml2 but we do have libxml2-2 in the container.
    rpm -i --nodeps work/aocc-compiler-5.1.0.sles15-1.x86_64.rpm
    ln -s /opt/AMD/aocc-compiler-5.1.0.sles15 /opt/AMD/aocc-compiler-5.1.0

    sed -e 's/5.0.0/5.1.0/g' -i /opt/cray/pe/lmod/modulefiles/core/aocc/5.1.0.lua

    #
    # Phase 4 - Other software that is not yet in the container but that we build upon.
    # 
    ...
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage libdrm-devel
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage libcurl-devel
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage zip unzip
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage perl-DBI # Installed on LUMI and used by some git command
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage libpciaccess-devel
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage hwloc-data
    # Some commands that we have found in build processes (and in particular WRF-SFIRE)
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage tcsh
    zypper --non-interactive --no-gpg-checks --no-refresh install --no-recommends --allow-downgrade --oldpackage time
    
    #
    # Phase 5 - Adding mount points so that future builds can even happen with advised mounts present
    #
